...
import { WebhookEngineModal } from "./components/WebhookEngineModal";
      ...
      <header className="min-h-14 sm:h-14 border-b border-white/10 bg-[#0A0A0A] flex flex-col sm:flex-row items-center justify-between px-3 sm:px-5 py-2.5 sm:py-0 gap-3 shrink-0">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3.5 w-full sm:w-auto shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* Webhook Engine Trigger */}
            <button
              type="button"
              onClick={() => setIsWebhookModalOpen(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 text-[10px] sm:text-xs font-mono font-bold transition-all shadow-[0_0_12px_rgba(16,185,129,0.15)] cursor-pointer"
              title="Configure Signal Webhook Engine for your trading bot"
            >
              <Webhook size={13} className="text-emerald-400" />
              <span className="hidden sm:inline">Webhook Engine</span>
                ...
                <div className="col-span-2 sm:col-span-1 flex items-center justify-between sm:justify-start gap-4">
                  ...
                  {activeAnalysis?.signal && activeAnalysis.signal !== "NO TRADE" && (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => setIsWebhookModalOpen(true)}
                        className="flex-1 sm:flex-none px-3.5 py-2 rounded-lg text-xs font-mono font-bold uppercase transition-all border border-emerald-500/40 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 shadow-lg flex items-center justify-center gap-1.5 active:scale-95 cursor-pointer whitespace-nowrap"
                        title="Send this complete signal to your webhook trading bot"
                      >
                        <Zap size={14} />
