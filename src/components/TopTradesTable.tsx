...
import { TrendingUp, TrendingDown, Minus, Activity, RefreshCw, Lock, Unlock, Zap, Check, Webhook } from 'lucide-react';
...
export const TopTradesTable: React.FC<TopTradesTableProps> = () => {
  ...
  const [dispatchingSymbol, setDispatchingSymbol] = useState<string | null>(null);
  const [dispatchedSymbols, setDispatchedSymbols] = useState<Record<string, boolean>>({});

  const handleQuickWebhookDispatch = async (s: any) => {
    try {
      setDispatchingSymbol(s.symbol);
      const isFrozen = !!frozenEntries[s.symbol];
      const entryPrice = isFrozen ? frozenEntries[s.symbol] : (s.analysis?.suggestedEntry || s.lastPrice);
      
      const payload = {
        signal: {
          symbol: s.symbol,
          direction: s.analysis?.signal || 'LONG',
          entryPrice,
          limitEntry: s.analysis?.limitEntry,
          sl: s.analysis?.sl || 0,
          tp1: s.analysis?.tp1,
          tp2: s.analysis?.tp2,
          tp3: s.analysis?.tp3 || s.analysis?.tp,
          confidence: s.analysis?.confidence || 90,
          analysis: s.analysis,
          timeframe: '15m',
          session: 'Scanner Grid',
        }
      };
      ...
      if (res.ok) {
        setDispatchedSymbols(prev => ({ ...prev, [s.symbol]: true }));
        setTimeout(() => {
          setDispatchedSymbols(prev => {
            const next = { ...prev };
      ...
      console.error('Failed to quick dispatch webhook:', err);
    } finally {
      setDispatchingSymbol(null);
    }
  };
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-white/40 text-[10px] font-mono uppercase tracking-widest border-b border-white/10">
                  ...
                  <th className="pb-3 px-4 font-semibold text-center">Lock</th>
                  <th className="pb-3 px-4 font-semibold text-center">Webhook</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={`${s.symbol}-${idx}`} className="group hover:bg-white/[0.02] transition-colors duration-150">
                      ...
                      <td className="py-4 px-4">
                        <div className="flex justify-center">
                          <button
                            onClick={() => handleQuickWebhookDispatch(s)}
                            disabled={dispatchingSymbol === s.symbol}
                            className={`p-1.5 px-2.5 rounded-md transition-all border shadow-sm flex items-center gap-1 text-[11px] font-mono font-bold hover:scale-105 active:scale-95 cursor-pointer
                              ${dispatchedSymbols[s.symbol]
                                ? 'bg-emerald-500 text-black border-emerald-400'
                                : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/20'
                              }`}
                            title="Dispatch complete signal to webhook trading bot"
                          >
                            {dispatchingSymbol === s.symbol ? (
                              <RefreshCw className="w-3 h-3 animate-spin" />
                            ) : dispatchedSymbols[s.symbol] ? (
                              <>
                  ...
                  <div className="flex items-center justify-between gap-3 pt-1">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleQuickWebhookDispatch(s)}
                        disabled={dispatchingSymbol === s.symbol}
                        className={`flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-[11px] font-mono font-bold border transition-all cursor-pointer hover:scale-105 active:scale-95
                          ${dispatchedSymbols[s.symbol]
                            ? 'bg-emerald-500 text-black border-emerald-400'
                            : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/20'
                          }`}
